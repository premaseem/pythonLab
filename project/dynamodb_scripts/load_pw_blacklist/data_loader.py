...
def load_bulk_data(input_file, dynamo_table):
    ...
    with dynamo_table.batch_writer() as batch:
        for row in items:
            insert_data_in_dynamodb(row, batch)


def insert_data_in_dynamodb(item, batch):
    try:
...
if __name__ == "__main__":
    ...
    logging.info(
        "$$$ Started uploading data from file {}".format(inputs.input_file_path))

    start_time = datetime.datetime.now()

    # load_data(inputs.input_file_path, table)
    load_bulk_data(inputs.input_file_path, table)
    ...
    logging.info("Time taken to upload in Seconds {}".format(time_to_upload.seconds))
    logging.info(
